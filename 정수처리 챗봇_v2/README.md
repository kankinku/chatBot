...
정수장 도메인에 특화된 PDF 기반 질문-답변 챗봇입니다. RAG(Retrieval-Augmented Generation) 기술을 활용하여 정수장 관련 문서에서 정확한 정보를 검색하고 답변을 생성합니다.
...
# 특정 질문 테스트
python scripts/autorun.py --backend auto --question "정수장의 주요 공정은 무엇인가요?"
```
...
# 4. 질문-답변 테스트
python scripts/manual_cli.py --corpus data/corpus_v1.jsonl --question "정수장의 주요 공정은 무엇인가요?"
```
...
curl -X POST "http://localhost:8000/api/qa/batch" \
  ...
  -d '{
    "items": [
      {"id": "1", "question": "정수장의 주요 공정은 무엇인가요?"},
      {"id": "2", "question": "여과 공정의 원리는 무엇인가요?"}
    ],
    "mode": "accuracy"
  }'
```
...
**정수장 챗봇 시스템** - 정확하고 신뢰할 수 있는 정수장 정보 제공