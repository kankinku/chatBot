...
def main():
    ...
    ap.add_argument("--ocr-dpi", type=int, default=300, help="OCR rasterization DPI (via fitz) - higher DPI for better accuracy")
    ap.add_argument("--chunking", default="window", choices=["paragraph", "page", "window"], help="Chunking strategy for extracted text")
    ap.add_argument("--chunk-size", type=int, default=500, help="Sliding window chunk size (window mode)")
    ap.add_argument("--chunk-overlap", type=int, default=100, help="Sliding window overlap (window mode)")
    ap.add_argument("--wt-disable", action="store_true", help="Disable domain-specific wastewater chunking presets")
    ...
    ap.add_argument("--llm-correct-disable", action="store_true", help="Disable local LLM post-correction for low-quality spans")
    ...
    # 중복 제거 옵션
    ap.add_argument("--dedup-disable", action="store_true", help="Disable chunk deduplication during corpus building")
    ...
    # 구버전 플래그(호환): --ocr-lite*
    ap.add_argument("--ocr-lite-disable", action="store_true", help="[deprecated] disable LLM correction")
    ...
    # 숫자 중심 확장 청크 옵션
    ap.add_argument("--numeric-expand-disable", action="store_true", help="Disable numeric-focused expanded chunks with neighbor context")
    ap.add_argument("--numeric-window", type=int, default=3, help="Neighbor lines to include on each side for numeric expansion")
    ap.add_argument("--numeric-emphasis", type=int, default=5, help="Repeat factor for numeric tokens to emphasize in retrieval")
    args = ap.parse_args()
    ...
    for pdf in pdf_dir.glob("**/*.pdf"):
        ...
        if args.chunking == "page":
            ...
            try:
                ...
                cfg = PDFChunkConfig(
                    ...
                    enable_wastewater_chunking=not args.wt_disable,
        ...
        use_llm_correction = not (args.llm_correct_disable or args.ocr_lite_disable)
        lc_threshold = args.llm_correct_threshold if not args.llm_correct_disable else args.ocr_lite_threshold
        lc_max_chars = args.llm_correct_max_chars if not args.llm_correct_disable else args.ocr_lite_max_chars
        lc_batch = args.llm_correct_batch if not args.llm_correct_disable else args.ocr_lite_batch
        ...
        if not args.numeric_expand_disable and lines:
            try:
                from unifiedpdf.measurements import extract_measurements
