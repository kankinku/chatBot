...
from config.constants import LLMConstraints
...
class MarketDataProvider:
    ...
    def analyze_market_with_llm(self):
        ...
        {LLMConstraints.COMMON_RULES}
        {LLMConstraints.ANALYSIS_RULES}

        [현재 시장 데이터]
        {data_text}
        
        [작성 가이드]
        - 내용: TGA, 지준금(Reserves), 금리를 종합하여 실제 시장 유동성이 늘었는지 줄었는지 판단할 것.
...
- ** 기호 사용 금지 (볼드 금지)
- 이모지 사용 금지
- 중국어, 일본어, 베트남어 등 외국어 사용 금지
- "분석 내용", "파급 효과" 같은 소제목 금지
