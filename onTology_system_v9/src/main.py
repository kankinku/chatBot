...
async def lifespan(app: FastAPI):
    ...
    - Startup: LLM 체크, Market Data 초기화, 지수 데이터 수집
    - Shutdown: 정리 작업
    """
    # === Startup ===
    logger.info(f"[Startup] Initializing {settings.APP_NAME} v5.0...")
    ...
    # 3. Market Indices 데이터 수집 (Index 페이지용)
    try:
        market_indices.initialize()
        logger.info("[Startup] Market indices initialized.")
    except Exception as e:
        logger.error(f"[Startup] Market Indices Init Failed: {e}")
...
def create_app() -> FastAPI:
    ...
    async def fundamental_view():
        """펀더멘털 분석"""
        return FileResponse(static_dir / "fundamental.html")
