...
class PDFKeywordExtractor:
    ...
    def extract_keywords_from_text(self, text: str) -> List[str]:
        ...
        # 상세 나열 대신 요약 로그: 상위 10개와 총 개수만
        if keywords:
            top_n = 10
            sample = keywords[:top_n]
            remainder = max(0, len(keywords) - top_n)
            logger.info(f"[SUCCESS] 키워드 추출 완료: 총 {len(keywords)}개 (상위 {top_n}: {sample}{' +'+str(remainder)+'개' if remainder else ''})")
        else:
            logger.info("[SUCCESS] 키워드 추출 완료: 0개")
        return keywords
