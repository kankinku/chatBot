...
class FastCache:
    ...
    def _generate_key(self, query: str, context: Union[str, Dict[str, Any]] = "") -> str:
        """
        ...
        if isinstance(context, dict):
            ...
            model_ver = context.get("model_version", "")
            qctx = context.get("context", "")
            combined = f"{user}|{route}|{model_ver}|{query}|{qctx}"
        else:
            combined = f"{query}|{context}"
        return hashlib.md5(combined.encode('utf-8')).hexdigest()

    @staticmethod
    def build_context(user_id: str = "", route: str = "", model_version: str = "", query_context: str = "") -> Dict[str, str]:
        return {
            "user_id": user_id or "",
            "route": route or "",
    ...
    def get(self, query: str, context: str = "") -> Optional[Any]:
        ...
        with self._lock:
            ...
            if key not in self.cache:
                self.misses += 1
                if unified_logger:
                    unified_logger.info("cache_miss", LogCategory.CACHE, "fast_cache", metadata={"key_prefix": key[:8]})
                return None
            ...
            if current_time - item.timestamp > item.ttl:
                ...
                if unified_logger:
                    unified_logger.info("cache_expired", LogCategory.CACHE, "fast_cache", metadata={"key_prefix": key[:8]})
                return None
            ...
            if unified_logger:
                unified_logger.info("cache_hit", LogCategory.CACHE, "fast_cache", metadata={"key_prefix": key[:8]})
            return item.data
    ...
    def put(self, query: str, data: Any, context: str = "", ttl: Optional[float] = None) -> None:
        ...
        with self._lock:
            ...
            if unified_logger:
                unified_logger.info("cache_put", LogCategory.CACHE, "fast_cache", metadata={"key_prefix": key[:8]})
