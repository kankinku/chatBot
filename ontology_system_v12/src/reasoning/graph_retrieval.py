...
Domain 우선, Personal 보조로 경로를 검색한다.
"""
import logging
...
from src.common.asof_context import get_context, validate_access
...
class GraphRetrieval:
    ...
    def retrieve(
        ...
        # as_of 가드: REPLAY 모드에서 필수
        ctx = get_context()
        # If explicitly passed as_of, validate against it (or ensure context matches)
        validation_time = as_of if as_of else ctx.as_of
        validate_access(validation_time)
