...
class PolicyWeightLearner:
    ...
    # ------------------------------------------------------------------ #
    def export_active_to_registry(self) -> str:
        """
        활성 PolicyConfig를 PolicyBundle 형태로 registry에 저장하고 ID를 반환.
        """
        active = self.get_active_policy()
        if not active:
            raise ValueError("No active policy to export")

        params = {
            "fusion": {
                "domain_weight": active.ees_weights.get("domain_weight", 0.7),
                "personal_weight": active.ees_weights.get("personal_weight", 0.3),
                "semantic_penalty": active.ees_weights.get("semantic_penalty", 0.0),
                "decay_lambda": active.ees_weights.get("decay_lambda", 0.1),
            },
            "routing": {
                "domain_threshold": active.thresholds.get("domain_candidate", 0.55),
                "personal_threshold": active.thresholds.get("personal_candidate", 0.35),
            },
            "retrieval": {
                "max_depth": active.ees_weights.get("max_depth", 4),
                "max_paths": active.ees_weights.get("max_paths", 10),
            },
        }

        bundle = PolicyBundle(
            policy_id=active.version,
            version=int(active.version.split("_")[-1]) if "_" in active.version else 1,
            created_at=datetime.utcnow(),
            params=params,
        )
        registry = get_policy_registry()
        registry_id = registry.save_policy_variant(
            parent_id=bundle.policy_id,
            params=bundle.params,
            metrics={"exported": True},
        )
        registry.set_active_policy(registry_id)
        return registry_id
