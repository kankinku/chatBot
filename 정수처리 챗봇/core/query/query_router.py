...
class QueryRouter:
    ...
    def __init__(self, embedding_model: str = "jhgan/ko-sroberta-multitask"):
        ...
        # 통합 설정에서 회사/프로젝트 정보 및 정책 버전 읽기
        from core.config.unified_config import get_config, get_config_version
        ...
        self.policy_version = get_config_version()
        
        logger.info(f"쿼리 라우터 초기화: {self.company_name} - {self.project_name} | policy={self.policy_version}")
        ...
        # 라우트 카운터
        self._route_counts = {r: 0 for r in QueryRoute}
    ...
    def route_query(self, question: str) -> RouteResult:
        ...
        Returns:
            라우팅 결과
        """
        # 우선 규칙: 특정 키워드가 있으면 즉시 분류 (승격/강등 이벤트 기준)
        ...
        # 특별한 우선순위 규칙: 정수장 관련 키워드가 있으면 PDF 검색으로 라우팅
        water_plant_keywords = ['정수장', '전력', '사용량', '소비량', '전기', '에너지', '운영', '관리', '시설', '장비']
        if any(keyword in question_lower for keyword in water_plant_keywords):
            result = RouteResult(
                route=QueryRoute.PDF_SEARCH,
                confidence=0.95,
                reasoning="우선순위 규칙: 정수장 관련 키워드 감지로 PDF 검색 라우팅"
            )
            try:
                from utils.unified_logger import unified_logger, LogCategory
                unified_logger.info("promotion_rule", LogCategory.SYSTEM, module="router", metadata={"reason": "water_keywords"})
            except Exception:
                pass
            self._route_counts[QueryRoute.PDF_SEARCH] = self._route_counts.get(QueryRoute.PDF_SEARCH, 0) + 1
            return result
        
        # "방법" 키워드가 있으면 PDF 검색으로 라우팅
        ...
        # 인사말이 가장 높은 경우
        if greeting_score > 0 and greeting_score >= pdf_score:
            res = RouteResult(
                route=QueryRoute.GREETING,
                confidence=0.9,
                reasoning=f"키워드 기반: 인사말 패턴 감지 (점수: {greeting_score})"
            )
            self._route_counts[QueryRoute.GREETING] = self._route_counts.get(QueryRoute.GREETING, 0) + 1
            return res
        ...
        if pdf_score > 0:
            res = RouteResult(
                route=QueryRoute.PDF_SEARCH,
                confidence=0.85,
                reasoning=f"키워드 기반: 기능/법률 질문 패턴 매칭 (점수: {pdf_score})"
            )
            self._route_counts[QueryRoute.PDF_SEARCH] = self._route_counts.get(QueryRoute.PDF_SEARCH, 0) + 1
            return res
        ...
        try:
            ...
            # 인사말이 가장 높은 경우
            if greeting_score > 0.5 and greeting_score > pdf_score:
                res = RouteResult(
                    route=QueryRoute.GREETING,
            ...
            if pdf_score > 0.35:
                res = RouteResult(
                    ...
                    reasoning=f"SBERT 기반: PDF 검색으로 분류 (유사도: {pdf_score:.3f})",
                    metadata={"scores": route_scores}
                )
                self._route_counts[QueryRoute.PDF_SEARCH] = self._route_counts.get(QueryRoute.PDF_SEARCH, 0) + 1
                return res
    ...
    def get_route_counters(self) -> Dict[str, int]:
        return {route.value: cnt for route, cnt in self._route_counts.items()}
