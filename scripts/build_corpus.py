...
# Canonical repository roots for direct CLI execution.
project_root = Path(__file__).resolve().parent.parent
src_root = project_root / "src"  # scripts의 부모 디렉토리
sys.path.insert(0, str(src_root))
sys.path.insert(0, str(project_root))

from chatbot.preprocessing.pdf_extractor import PDFExtractor
from chatbot.preprocessing.text_cleaner import TextCleaner
from chatbot.preprocessing.ocr_corrector import OCRCorrector
from chatbot.chunking.sliding_window_chunker import SlidingWindowChunker
from chatbot.chunking.numeric_chunker import NumericChunker
...
def process_pdf(
    ...
    # 청킹 설정
    from chatbot.chunking.base_chunker import ChunkingConfig
...
def deduplicate_chunks(chunks: List[Chunk]) -> List[Chunk]:
    ...
    from chatbot.core.types import RetrievedSpan
