...
class SearchConfig:
    """검색 설정"""
    dense_k: int = 200          # Dense 검색 후보 수
    bm25_k: int = 200          # BM25 검색 후보 수
    rrf_k: int = 200           # RRF 병합 후 후보 수
    mmr_k: int = 80            # MMR 후 최종 후보 수
    mmr_lambda: float = 0.7    # MMR 다양성 파라미터 (0~1, 높을수록 관련성 우선)
    rrf_constant: int = 60     # RRF 상수 (일반적으로 60 사용)
    multiview_enabled: bool = True  # 멀티뷰 검색 사용 여부
