...
SETTINGS_PATH = REPO_ROOT / "services" / "gateway" / "chatbot_backend" / "settings.py"
ENV_EXAMPLE_PATH = REPO_ROOT / "services" / "gateway" / "env.example"
COMPOSE_PATH = REPO_ROOT / "deploy" / "compose" / "docker-compose.yml"
VALID_DATABASE_PASSWORD = "Q7mR2xV9kL4pN8dT6wC3zH5sJ1fB0uY8eA6iO4nP2rS9vX7cD5qG6hM3"
CONFIGURATION_SECURITY_PATH = (
    REPO_ROOT / "services" / "gateway" / "chatbot_backend" / "configuration_security.py"
)
WAIT_FOR_MYSQL_PATH = REPO_ROOT / "services" / "gateway" / "wait-for-mysql.py"
...
def _run_settings_import(values=None, unset=()):
    ...
    with tempfile.TemporaryDirectory() as isolated_cwd:
        ...
        shutil.copytree(
            REPO_ROOT / "services" / "gateway" / "chatbot_backend",
...
def test_gateway_does_not_depend_on_mysql_root_credentials():
    settings_source = SETTINGS_PATH.read_text(encoding="utf-8")
    compose_source = COMPOSE_PATH.read_text(encoding="utf-8")
    assert "MYSQL_ROOT_PASSWORD = config(" not in settings_source
    assert "mysql_root_password=" not in settings_source

    import yaml
    services = yaml.safe_load(compose_source)["services"]
