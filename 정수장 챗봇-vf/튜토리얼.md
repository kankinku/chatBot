...
4. 수동 절차(원한다면)
- PDF → 코퍼스(JSONL) 생성
  - `python scripts/build_corpus_from_pdfs.py --pdf_dir pdfs --out data/corpus_v1.jsonl --pdf-extractor auto --ocr auto --ocr-lang kor+eng --ocr-dpi 200`
- 코퍼스 → 벡터 인덱스 생성
  - FAISS: `python scripts/build_vector_index.py --corpus data/corpus_v1.jsonl --backend faiss --outdir vector_store`
  - HNSW: `python scripts/build_vector_index.py --corpus data/corpus_v1.jsonl --backend hnsw --outdir vector_store`
- 수동 CLI(질문 테스트)
  - `python scripts/manual_cli.py --corpus data/corpus_v1.jsonl --mode accuracy --store-backend auto --question "<질문>"`
- 하네스(벤치마크)
  - `python scripts/run_qa_benchmark.py --input data/tests/qa.json --corpus data/corpus_v1.jsonl --mode accuracy --store-backend auto --report out/report.json --csv out/report.csv`
