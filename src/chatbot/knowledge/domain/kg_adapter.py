"""
Domain KG adapter for storage integration.
"""
import json
import logging
...
from chatbot.knowledge.settings import get_settings
from chatbot.knowledge.storage.graph_repository import GraphRepository
from chatbot.knowledge.storage.transaction_manager import KGTransactionManager, Transaction
...
class DomainKGAdapter:
    """
    Adapter for reading/writing domain relations to a graph repository.
    """
    ...
    def load_domain_data(self) -> None:
        """Load canonical domain data into the repository."""
        domain_path = self._settings.resolve_path(self._settings.store.domain_data_path)
    ...
    def upsert_relation(
        ...
        """Upsert a relation into the repository."""
        ...
        rel_props = {
            ...
            "support_score": relation.support_score,
            "conflict_score": relation.conflict_score,
            "support_source_count": relation.support_source_count,
            "conflict_source_count": relation.conflict_source_count,
            "evidence_score_version": relation.evidence_score_version,
    ...
    def get_relation(
        ...
        """Get a relation by endpoints and type."""
    ...
    def get_neighbors(self, entity_id: str, direction: str = "out") -> List[Dict]:
        """Get neighbors and un-scope domain relation types."""
    ...
    def delete_relation(
        ...
        if self._read_only and not force:
            logger.warning("Blocked attempt to delete Domain relation (read-only)")
            return False
    ...
    def _props_to_relation(
        ...
        """Convert stored relation props into a DynamicRelation."""
        ...
        return DynamicRelation(
            ...
            support_score=float(props.get("support_score", 0.0)),
            conflict_score=float(props.get("conflict_score", 0.0)),
            support_source_count=int(props.get("support_source_count", 0)),
            conflict_source_count=int(props.get("conflict_source_count", 0)),
            evidence_score_version=props.get("evidence_score_version", "legacy"),
