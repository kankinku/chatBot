...
def load_config(config_path: Optional[str] = None) -> dict:
    ...
    if config_path is None:
        config_path = get_settings().get_config_path("infrastructure")
...
from chatbot.knowledge.llm.llm_client import LLMClient
from chatbot.knowledge.llm.gateway import LLMGateway
...
def build_llm_client(config: Optional[dict] = None) -> LLMClient:
    ...
    if backend == "ollama":
        from chatbot.knowledge.llm.ollama_adapter import OllamaLLMClient
        ...
        from chatbot.knowledge.llm.ollama_adapter import MockLLMClient
...
def get_domain_kg_adapter():
    ...
    if _domain_adapter is None:
        ...
        _domain_adapter = DomainKGAdapter(
            ...
            read_only=False,  # 테스트 및 학습 파이프라인에서 쓰기 허용
        )
...
def reset_all() -> None:
    """모든 싱글톤 리셋"""
    global _graph_repo, _tx_manager, _llm_gateway, _domain_adapter
    _graph_repo = None
    _tx_manager = None
    _llm_gateway = None
    _domain_adapter = None
